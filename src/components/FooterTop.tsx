/**
 * Upper footer block — address, navigation links, and social icons.
 * Figma node: Section/Footer Top (0:682)
 *
 * Uses the same blue gradient as the newsletter and ticket sections.
 * Left column: `<address>` with postal address, email, phone, and an
 * attribution paragraph for the SNB / Bernisches Historisches Museum.
 * Right column: two `<ul>` link columns plus a social-icon list.
 *
 * NOTE: Social icon assets are served by the Figma MCP server and expire in
 * 7 days. Replace with permanent SVG files before production.
 */

/** Social icon asset URLs from Figma MCP (expire in 7 days). */
...
export default function FooterTop() {
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between px-5 py-10 md:px-10 lg:px-20 lg:py-20 gap-10 lg:gap-0">
        {/* Left: contact info */}
        <address className="not-italic flex flex-col gap-4 text-white w-full lg:w-[427px]">
          <div className="flex flex-col gap-1 text-[16px] md:text-[19px] font-medium leading-[1.4]" style={{ fontFamily: font }}>
            ...
            <ul key={i} className="flex flex-col gap-3">
              {col.links.map(link => (
                <li key={link}>
                  <a
                    href="#"
                    className="text-[14px] md:text-[16px] font-semibold text-white leading-[1.5] hover:opacity-70 transition-opacity"
